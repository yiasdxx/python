...
def cross_entropy_error(y, t):
    ...
    参数:
        y: 模型输出概率 (batch_size, num_classes)
        t: 标签，可以是:
           1) 硬标签整数索引 (batch_size,)
           2) one-hot编码硬标签 (batch_size, num_classes)
           3) 软标签概率分布 (batch_size, num_classes)
    ...
    if t.ndim == 1:
        # 情况1: 硬标签整数索引 (batch_size,)
        # 例如: [0, 2, 1, 3]
        pass  # 保持is_soft_label=False

    elif t.ndim == 2 and t.shape[1] == y.shape[1]:
        # 情况2和3: 需要进一步检测

        # 检查是否是one-hot编码（硬标签的特殊情况）
        is_one_hot = True

        # 检查每一行是否只有一个1，其他都是0
        row_sums = np.sum(t, axis=1)
        row_max = np.max(t, axis=1)

        for i in range(batch_size):
            if not (np.abs(row_sums[i] - 1.0) < 1e-6 and
                    (np.abs(row_max[i] - 1.0) < 1e-6 or
                     np.abs(row_max[i] - 0.0) < 1e-6)):
                is_one_hot = False
                break

        if not is_one_hot:
            # 不是one-hot，检查是否为概率分布（软标签）
            # 软标签特征：每行和为1，值在0-1之间
            row_sums = np.sum(t, axis=1)
            is_prob_dist = True
            ...
            for i in range(batch_size):
                # 检查行和是否接近1
                if not (0.999 < row_sums[i] < 1.001):
                    is_prob_dist = False
                    break
                # 检查值是否在合理范围内
                if np.any(t[i] < -0.001) or np.any(t[i] > 1.001):
                    is_prob_dist = False
                    break
    ...
    if is_soft_label:
        # 软标签：使用概率分布的交叉熵
        # 添加小值防止数值问题
        t_safe = np.clip(t, 1e-15, 1 - 1e-15)
        y_safe = np.clip(y, 1e-15, 1 - 1e-15)

        loss = -np.sum(t_safe * np.log(y_safe)) / batch_size
        return loss, True  # 返回损失和标签类型标识
        ...
        # 防止数值问题
        y_safe = np.clip(y[np.arange(batch_size), t], 1e-15, 1 - 1e-15)

        loss = -np.sum(np.log(y_safe)) / batch_size
        return loss, False  # 返回损失和标签类型标识
...
class SoftmaxWithLoss:
    """智能检测软硬标签的Softmax与损失层"""

    def __init__(self):
        self.loss = None
        self.y = None  # softmax输出
        self.t = None  # 原始标签
        self.is_soft_label = False  # 检测到的标签类型
        self.t_hard = None  # 硬标签版本（如果需要）

    def forward(self, x, t):
        """前向传播，自动检测标签类型"""
        self.t = t.copy()  # 保存原始标签
        self.y = softmax(x)

        # 使用智能检测计算损失
        self.loss, self.is_soft_label = cross_entropy_error(self.y, self.t)

        # 如果是硬标签，保存整数索引版本
        if not self.is_soft_label:
            if t.ndim == 2:
                self.t_hard = t.argmax(axis=1)
            else:
                self.t_hard = t.astype(np.int64).flatten()
    ...
    def backward(self, dout=1):
        ...
        if self.is_soft_label:
            ...
            # 硬标签梯度：∂L/∂x = y - one_hot(t)
            dx = self.y.copy()
            dx[np.arange(batch_size), self.t_hard] -= 1
...
class Trainer:
    ...
    def __init__(self, network, x_train, t_train, x_val, t_val,
        ...
        # 标签类型统计
        self.soft_label_count = 0
        self.hard_label_count = 0
    ...
    def augment_batch(self, x_batch, t_batch=None):
        ...
        # 应用CutMix增强（如果需要）
        if self.use_cutmix and t_batch is not None and random.random() < self.augmenter.cutmix.prob:
            try:
                # 确保标签是合适的格式
                if t_batch.ndim == 1:
                    # 整数标签 -> 转换为one-hot用于CutMix
                    num_classes = int(t_batch.max() + 1)
                    t_batch_onehot = np.eye(num_classes)[t_batch.astype(int)]
                elif t_batch.ndim == 2 and t_batch.shape[1] > 1:
                    t_batch_onehot = t_batch
                else:
                    # 无法处理的格式，跳过CutMix
                    print(f"⚠️ 无法处理的标签形状: {t_batch.shape}，跳过CutMix")
                    return x_augmented, t_batch

                # 应用CutMix
                x_augmented, t_batch = self.augmenter.apply_cutmix(x_augmented, t_batch_onehot)
    ...
    def train_step(self):
        ...
        # 统计标签类型
        if hasattr(self.network.last_layer, 'is_soft_label'):
            if self.network.last_layer.is_soft_label:
                self.soft_label_count += 1
            else:
                self.hard_label_count += 1
        ...
        if self.current_iter % self.iter_per_epoch == 0:
            ...
            # 显示标签统计
            total_batches = self.soft_label_count + self.hard_label_count
            soft_ratio = 0
            if total_batches > 0:
                soft_ratio = self.soft_label_count / total_batches * 100
    ...
    def train(self):
        ...
        print(f"使用CutMix: {self.use_cutmix}")
        if self.use_cutmix:
            print(f"CutMix概率: {self.augmenter.cutmix.prob if self.augmenter else 'N/A'}")
        print(f"软标签支持: {'自动检测' if hasattr(self.network.last_layer, 'is_soft_label') else '未知'}\n")
    ...
    def get_label_statistics(self):
        """获取标签类型统计信息"""
        total = self.soft_label_count + self.hard_label_count
        if total == 0:
            return {"soft": 0, "hard": 0, "soft_ratio": 0}

        return {
            "soft": self.soft_label_count,
            "hard": self.hard_label_count,
            "soft_ratio": self.soft_label_count / total * 100,
            "total_batches": total
        }